...
requires a != null
ensures b == (counte(a[0..a.Length], key) == 1)
ensures forall i :: 0 <= i < a.Length ==> (a[i] == old(a[i]))
{
    ...
    invariant count == counte(a[0..i], key)
    {
        if(a[i] == key)
        {
            count := count + 1;
...
function counte(a: seq<int>, key: int): int 
ensures counte(a, key) == multiset(a)[key]
{
    multiset(a)[key]
}
...
method Main()
{
    ...
    assert counte(testArray1[0..testArray1.Length], 1) == 3;
    assert !test1;


    var test2 := Just1(testArray1, 2);
    assert counte(testArray1[0..testArray1.Length], 2) == 1;
    assert test2;
    
    var test3 := Just1(testArray1, 3);
    assert counte(testArray1[0..testArray1.Length], 3) == 0;
    assert !test3;

    var test4 := Just1(testArray2, 1);
    assert counte(testArray2[0..testArray2.Length], 1) == 1;
    assert test4;

    var test5 := Just1(testArray2, 2);
    assert counte(testArray2[0..testArray2.Length], 2) == 0;
    assert !test5;

    var test6 := Just1(testArray3, 1);
    assert counte(testArray3[0..testArray3.Length], 1) == 0;
    assert !test6;
